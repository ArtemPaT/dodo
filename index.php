...
<html lang="en">
  ...
  <body>
    <div class="col-10 mx-auto">     
        <div class="row">   <!-- header -->
            ...
            <div class="col-7">
                <div class="row">
                    <div class="col-2 pt-4">
                        <a href="#pizza" class="text-center text-dark"><h5>Пицца</h5></a>
                    </div>
                    <div class="col-2 pt-4">
                        <a href="#combo" class="text-center text-dark"><h5>Комбо</h5></a>
                    </div>
                    <div class="col-2 pt-4">
                        <a href="#desert" class="text-center text-dark"><h5>Десерты</h5></a>
                    </div>
                </div>
            </div>
            ...
            <div class="bsk-box bg-white border rounded col-3 pt-3"> <!-- Корзина -->
             <?php 
                ...
                $con_table2 = mysqli_query($con_bd, 'SELECT * FROM trash');
                $table_count2 = mysqli_num_rows($con_table2);
                $ress = mysqli_query($con_bd, 'SELECT SUM(price) AS total FROM trash');
                $lol1 = $ress -> fetch_assoc();
                for ($i=0; $i < $table_count2; $i++) { 
                    $result2 = $con_table2 -> fetch_assoc();
                    
              ?>
                <div class="row">
                    <div class="col-3">
                        <img src="img/<?php echo $result2["img"] ?>" class="w-100">
                    </div>
                    <div class="col-4 mt-2">
                        <h5><?php echo $result2["name"] ?></h5>                        
                    </div>
                    <div class="col-2 mt-2 px-0 text-center">
                        <h5><?php echo $result2["price"] ?> <span>₽</span></h5>                        
                    </div>
                    <div class="col-2 mt-2">
                   
                    <form action="delete.php" method="GET">
                        <input style="display: none;" type="" name="id" value="<?php echo $result2["id"] ?>">
             ...
             <?php } ?>
                ...
                <h5>Всего: <?php echo $lol1["total"]; ?>₽</h5>
                <div class="row">
                     <button class="btn bg-btn-dodo cls my-3 ml-3">Закрыть</button>
                    <form action="clear.php">
                        <button class="btn bg-btn-dodo my-3 ml-5">Очистить</button>
                    </form>
                </div>
        ...
        <div class="row pb-5">
            <?php 
                $con_table = mysqli_query($con_bd, 'SELECT * FROM products');
                $table_count = mysqli_num_rows($con_table);
                for ($i=0; $i < $table_count; $i++) { 
                    $result = $con_table -> fetch_assoc();
             ?>
            <div class="col-3 mt-5">   
                <img name="" src="img/<?php echo $result['img'] ?>" class="w-100">
                <div class="pizz"> 
                    <h3><?php echo $result['name'] ?></h3>   
                    <p class="text-secondary"><?php echo $result['text'] ?></p>
                </div>
